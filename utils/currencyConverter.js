...
export const convertCurrency = async (amount, fromCurrency, toCurrency = BASE_CURRENCY) => {
    try {
        ...
        let response;
        try {
            response = await axiosInstance.get(`https://v6.exchangerate-api.com/v6/${API_KEY}/pair/${fromCurrency}/${toCurrency}`);
        } catch (error) {
            if (error.response) {
...
const handleApiError = (errorData) => {
    ...
    switch (errorType) {
        ...
        case "malformed-request":
            throw new Error("The request structure is invalid. Please check the request format.");
        case "invalid-key":
            throw new Error("The provided API key is invalid.");
        case "inactive-account":
            throw new Error("Your account is inactive. Please confirm your email address.");
        case "quota-reached":
            throw new Error("Your account has reached the maximum number of requests allowed by your plan.");
        case "unknown-code":
        default:
            throw new Error("An unknown error occurred. Please refer to the API documentation.");
    }
};
